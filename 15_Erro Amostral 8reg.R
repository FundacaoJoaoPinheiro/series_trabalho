...
rho_d <- c(rho1_d_bh, rho2_d_bh, rho3_d_bh, rho4_d_bh, rho5_d_bh)
R_d <- matrix(1, nrow = 5, ncol = 5)  
for (i in 1:5) {
  for (j in 1:5) {
    if (i != j) {
...
rho_o <- c(rho1_o_bh, rho2_o_bh, rho3_o_bh, rho4_o_bh, rho5_o_bh)
R_o <- matrix(1, nrow = 5, ncol = 5)
for (i in 1:5) {
  for (j in 1:5) {
    if (i != j) {
...
ar5_o_bh<-as.data.frame(phi_o)

par_ar5_bh<-cbind(ar5_d_bh,ar5_o_bh)
...
params_bh <- list("dbbh"=dbbh,"calculos_desocupada_bh"=clc_d_bh,
                ...
                "mod_ma1"=par_ma1_bh,"mod_ma2"=par_ma2_bh, "mod_arma11"=par_arma11_bh, "mod_ar5"=par_ar5_bh)

saveRDS(params_bh,file = "D:/FJP2425/Programacao/data/pseudoerros_8reg/01_params_bh.rds")
...
rho_d <- c(rho1_d_ent, rho2_d_ent, rho3_d_ent, rho4_d_ent, rho5_d_ent)
R_d <- matrix(1, nrow = 5, ncol = 5)  # Matriz de autocorrelações
for (i in 1:5) {
  for (j in 1:5) {
    if (i != j) {
      R_d[i, j] <- rho_d[abs(i - j)]
...
rho_o <- c(rho1_o_ent, rho2_o_ent, rho3_o_ent, rho4_o_ent, rho5_o_ent)
R_o <- matrix(1, nrow = 5, ncol = 5)
for (i in 1:5) {
  for (j in 1:5) {
    if (i != j) {
...
rho_o_vec <- rho_o # Vetor
phi_o <- solve(R_o, rho_o_vec)
ar5_o_ent<-as.data.frame(phi_o)

par_ar5_ent<-cbind(ar5_d_ent,ar5_o_ent)
...
params_ent <- list("dbent"=dbent,"calculos_desocupada_ent"=clc_d_ent,
                   ...
                   "mod_ma1"=par_ma1_ent,"mod_ma2"=par_ma2_ent, "mod_arma11"=par_arma11_ent, "mod_ar5"=par_ar5_ent)

saveRDS(params_ent,file = "D:/FJP2425/Programacao/data/pseudoerros_8reg/02_params_ent.rds")
...
rho_d <- c(rho1_d_sul, rho2_d_sul, rho3_d_sul, rho4_d_sul, rho5_d_sul)
R_d <- matrix(1, nrow = 5, ncol = 5)
for (i in 1:5) {
  for (j in 1:5) {
    if (i != j) {
...
rho_o <- c(rho1_o_sul, rho2_o_sul, rho3_o_sul, rho4_o_sul, rho5_o_sul)
R_o <- matrix(1, nrow = 5, ncol = 5)
for (i in 1:5) {
  for (j in 1:5) {
    if (i != j) {
...
rho_o_vec <- rho_o # Vetor
phi_o <- solve(R_o, rho_o_vec)
ar5_o_sul<-as.data.frame(phi_o)

par_ar5_sul<-cbind(ar5_d_sul,ar5_o_sul)
...
params_sul <- list("dbsul"=dbsul,"calculos_desocupada_sul"=clc_d_sul,
                   ...
                   "mod_ma1"=par_ma1_sul,"mod_ma2"=par_ma2_sul, "mod_arma11"=par_arma11_sul, "mod_ar5"=par_ar5_sul)

saveRDS(params_sul,file = "D:/FJP2425/Programacao/data/pseudoerros_8reg/03_params_sul.rds")
...
rho_d <- c(rho1_d_trg, rho2_d_trg, rho3_d_trg, rho4_d_trg, rho5_d_trg)
R_d <- matrix(1, nrow = 5, ncol = 5)
for (i in 1:5) {
  for (j in 1:5) {
    if (i != j) {
...
rho_o <- c(rho1_o_trg, rho2_o_trg, rho3_o_trg, rho4_o_trg, rho5_o_trg)
R_o <- matrix(1, nrow = 5, ncol = 5)
for (i in 1:5) {
  for (j in 1:5) {
    if (i != j) {
...
rho_o_vec <- rho_o # Vetor
phi_o <- solve(R_o, rho_o_vec)
ar5_o_trg<-as.data.frame(phi_o)

par_ar5_trg<-cbind(ar5_d_trg,ar5_o_trg)
...
params_trg <- list("dbtrg"=dbtrg,"calculos_desocupada_trg"=clc_d_trg,
                   ...
                   "mod_ma1"=par_ma1_trg,"mod_ma2"=par_ma2_trg, "mod_arma11"=par_arma11_trg, "mod_ar5"=par_ar5_trg)

saveRDS(params_trg,file = "D:/FJP2425/Programacao/data/pseudoerros_8reg/04_params_trg.rds")
...
rho_d <- c(rho1_d_mat, rho2_d_mat, rho3_d_mat, rho4_d_mat, rho5_d_mat)
R_d <- matrix(1, nrow = 5, ncol = 5)
for (i in 1:5) {
  for (j in 1:5) {
    if (i != j) {
...
rho_o <- c(rho1_o_mat, rho2_o_mat, rho3_o_mat, rho4_o_mat, rho5_o_mat)
R_o <- matrix(1, nrow = 5, ncol = 5)
for (i in 1:5) {
  for (j in 1:5) {
    if (i != j) {
...
rho_o_vec <- rho_o # Vetor
phi_o <- solve(R_o, rho_o_vec)
ar5_o_mat<-as.data.frame(phi_o)

par_ar5_mat<-cbind(ar5_d_mat,ar5_o_mat)
...
params_mat <- list("dbmat"=dbmat,"calculos_desocupada_mat"=clc_d_mat,
                   ...
                   "mod_ma1"=par_ma1_mat,"mod_ma2"=par_ma2_mat, "mod_arma11"=par_arma11_mat, "mod_ar5"=par_ar5_mat)

saveRDS(params_mat,file = "D:/FJP2425/Programacao/data/pseudoerros_8reg/05_params_mat.rds")

### 06-NORTE DE MINAS GERAIS ###################################################
  # Agora o Norte de Minas também incluí os dados da RIDE
...
rho_d <- c(rho1_d_nrt, rho2_d_nrt, rho3_d_nrt, rho4_d_nrt, rho5_d_nrt)
R_d <- matrix(1, nrow = 5, ncol = 5)
for (i in 1:5) {
  for (j in 1:5) {
    if (i != j) {
...
rho_o <- c(rho1_o_nrt, rho2_o_nrt, rho3_o_nrt, rho4_o_nrt, rho5_o_nrt)
R_o <- matrix(1, nrow = 5, ncol = 5)
for (i in 1:5) {
  for (j in 1:5) {
    if (i != j) {
...
rho_o_vec <- rho_o # Vetor
phi_o <- solve(R_o, rho_o_vec)
ar5_o_nrt<-as.data.frame(phi_o)

par_ar5_nrt<-cbind(ar5_d_nrt,ar5_o_nrt)
...
params_nrt <- list("dbnrt"=dbnrt,"calculos_desocupada_nrt"=clc_d_nrt,
                   ...
                   "mod_ma1"=par_ma1_nrt,"mod_ma2"=par_ma2_nrt, "mod_arma11"=par_arma11_nrt, "mod_ar5"=par_ar5_nrt)

saveRDS(params_nrt,file = "D:/FJP2425/Programacao/data/pseudoerros_8reg/06_params_nrt.rds")

### 07-VALE DO RIO DOCE ########################################################

dbrio<-base$`07-Vale do Rio Doce`
...
rho_d <- c(rho1_d_rio, rho2_d_rio, rho3_d_rio, rho4_d_rio, rho5_d_rio)
R_d <- matrix(1, nrow = 5, ncol = 5)
for (i in 1:5) {
  for (j in 1:5) {
    if (i != j) {
...
rho_o <- c(rho1_o_rio, rho2_o_rio, rho3_o_rio, rho4_o_rio, rho5_o_rio)
R_o <- matrix(1, nrow = 5, ncol = 5)
for (i in 1:5) {
  for (j in 1:5) {
    if (i != j) {
...
rho_o_vec <- rho_o # Vetor
phi_o <- solve(R_o, rho_o_vec)
ar5_o_rio<-as.data.frame(phi_o)

par_ar5_rio<-cbind(ar5_d_rio,ar5_o_rio)
...
params_rio <- list("dbrio"=dbrio,"calculos_desocupada_rio"=clc_d_rio,
                   ...
                   "mod_ma1"=par_ma1_rio,"mod_ma2"=par_ma2_rio, "mod_arma11"=par_arma11_rio, "mod_ar5"=par_ar5_rio)

saveRDS(params_rio,file = "D:/FJP2425/Programacao/data/pseudoerros_8reg/07_params_rio.rds")
...
rho_d <- c(rho1_d_cen, rho2_d_cen, rho3_d_cen, rho4_d_cen, rho5_d_cen)
R_d <- matrix(1, nrow = 5, ncol = 5)
for (i in 1:5) {
  for (j in 1:5) {
    if (i != j) {
...
rho_o <- c(rho1_o_cen, rho2_o_cen, rho3_o_cen, rho4_o_cen, rho5_o_cen)
R_o <- matrix(1, nrow = 5, ncol = 5)
for (i in 1:5) {
  for (j in 1:5) {
    if (i != j) {
...
rho_o_vec <- rho_o # Vetor
phi_o <- solve(R_o, rho_o_vec)
ar5_o_cen<-as.data.frame(phi_o)

par_ar5_cen<-cbind(ar5_d_cen,ar5_o_cen)
...
params_cen <- list("dbcen"=dbcen,"calculos_desocupada_cen"=clc_d_cen,
                   ...
                   "mod_ma1"=par_ma1_cen,"mod_ma2"=par_ma2_cen, "mod_arma11"=par_arma11_cen, "mod_ar5"=par_ar5_cen)

saveRDS(params_cen,file = "D:/FJP2425/Programacao/data/pseudoerros_8reg/08_params_cen.rds")
...
dbmg$media_ocupada = dbmg %>%  select(starts_with("ocupada")) %>%  
  replace(.==0, NA) %>% rowMeans(.,na.rm = TRUE)
dbmg$media_desocupada = dbmg %>%  select(starts_with("desocupada")) %>%  
  replace(.==0, NA) %>%  rowMeans(.,na.rm = TRUE)
...
dbmg$pseudo1_ocupada = dbmg$ocupada_1 - dbmg$media_ocupada
dbmg$pseudo2_ocupada = dbmg$ocupada_2 - dbmg$media_ocupada
dbmg$pseudo3_ocupada = dbmg$ocupada_3 - dbmg$media_ocupada
dbmg$pseudo4_ocupada = dbmg$ocupada_4 - dbmg$media_ocupada
dbmg$pseudo5_ocupada = dbmg$ocupada_5 - dbmg$media_ocupada

dbmg$pseudo1_desocupada = dbmg$desocupada_1 - dbmg$media_desocupada
dbmg$pseudo2_desocupada = dbmg$desocupada_2 - dbmg$media_desocupada
dbmg$pseudo3_desocupada = dbmg$desocupada_3 - dbmg$media_desocupada
dbmg$pseudo4_desocupada = dbmg$desocupada_4 - dbmg$media_desocupada
dbmg$pseudo5_desocupada = dbmg$desocupada_5 - dbmg$media_desocupada
...
lag = c(0:24)
clc_o_mg=as.data.frame(lag)
head(clc_o_mg)

## Calculo autocov dos pseudoerros (Ch)

clc_o_mg$Ch1 = Pcov2(dbmg$pseudo1_ocupada, lag = lags + 1)
clc_o_mg$Ch2 = Pcov2(dbmg$pseudo2_ocupada, lag = lags + 1)
clc_o_mg$Ch3 = Pcov2(dbmg$pseudo3_ocupada, lag = lags + 1)
clc_o_mg$Ch4 = Pcov2(dbmg$pseudo4_ocupada, lag = lags + 1)
clc_o_mg$Ch5 = Pcov2(dbmg$pseudo5_ocupada, lag = lags + 1)

View(clc_o_mg)

# Soma das autocovs dos pseudo-erros:

clc_o_mg$SomaChk = clc_o_mg$Ch1 + clc_o_mg$Ch2 +clc_o_mg$Ch3 +clc_o_mg$Ch4 +clc_o_mg$Ch5
clc_o_mg$autocov = clc_o_mg$SomaChk/(K^2-K)

# FAC
clc_o_mg$fac = clc_o_mg$SomaChk/clc_o_mg$SomaChk[1]

# FACP
clc_o_mg$facp = 0 
clc_o_mg$facp[2:25] = facp_acf(clc_o_mg$fac,lags)

# Estatísticas de teste:
## Referência: pg.86 de Silva e Cruz(2002) (anotação retirada do arqv original do Caio)

clc_o_mg$esttest = nrow(dbmg)*clc_o_mg$facp ^ 2 
clc_o_mg$pvalor = teste(clc_o_mg$facp,nrow(dbmg))
View(clc_o_mg)
...
{par(mfrow=c(1,2),mar=c(5,5,2,5),cex=0.8)
  par(bty = "l",cex.lab = 2)
  plot(x = clc_o_mg$lag, clc_o_mg$fac, xlab = "", ylab = "",
       type = "h", axes = FALSE, ylim = c(-2/sqrt(T) - 0.1, 1), lwd = 2, main = "FAC")
  lines(x = c(-3,lags), y = c(0,0),lwd = 2)
  lines(x = c(-3,lags), y = c(-1/T-2/sqrt(T), -1/T-2/sqrt(T)), col = "red", lty = 2, lwd = 2)
  lines(x = c(-3,lags), y = c(-1/T+2/sqrt(T), -1/T+2/sqrt(T)), col = "red", lty = 2, lwd = 2)
  ...
  par(bty = "l",cex.lab = 2)
  plot(x = clc_o_mg$lag[2:25], clc_o_mg$facp[2:25], xlab = "", ylab = "",
       type = "h", axes = FALSE, ylim = c(-2/sqrt(T) - 0.1, 1), lwd = 2, main = "FACP")
  lines(x = c(-3,lags), y = c(0,0),lwd = 2)
  lines(x = c(-3,lags), y = c(-2/sqrt(T),-2/sqrt(T)), col = "red", lty = 2, lwd = 2)
  lines(x = c(-3,lags), y = c(2/sqrt(T), 2/sqrt(T)), col = "red", lty = 2, lwd = 2)
...
lag = c(0:24)
clc_d_mg=as.data.frame(lag)
head(clc_d_mg)

## Calculo autocov dos pseudoerros (Ch)

clc_d_mg$Ch1 = Pcov2(dbmg$pseudo1_desocupada, lag = lags + 1)
clc_d_mg$Ch2 = Pcov2(dbmg$pseudo2_desocupada, lag = lags + 1)
clc_d_mg$Ch3 = Pcov2(dbmg$pseudo3_desocupada, lag = lags + 1)
clc_d_mg$Ch4 = Pcov2(dbmg$pseudo4_desocupada, lag = lags + 1)
clc_d_mg$Ch5 = Pcov2(dbmg$pseudo5_desocupada, lag = lags + 1)

View(clc_d_mg)

# Soma das autocovs dos pseudo-erros:

clc_d_mg$SomaChk = clc_d_mg$Ch1 + clc_d_mg$Ch2 +clc_d_mg$Ch3 +clc_d_mg$Ch4 +clc_d_mg$Ch5
clc_d_mg$autocov = clc_d_mg$SomaChk/(K^2-K)

# FAC
clc_d_mg$fac = clc_d_mg$SomaChk/clc_d_mg$SomaChk[1]
View(clc_d_mg)

# FACP
clc_d_mg$facp = 0 
clc_d_mg$facp[2:25] = facp_acf(clc_d_mg$fac,lags)
View(clc_d_mg)

# Estatísticas de teste:
## Referência: pg.86 de Silva e Cruz(2002) (anotação retirada do arqv original do Caio)
clc_d_mg$esttest = nrow(dbmg)*clc_d_mg$facp ^ 2 
clc_d_mg$pvalor = teste(clc_d_mg$facp,nrow(dbmg))
View(clc_d_mg)
...
{par(mfrow=c(1,2),mar=c(5,5,2,5),cex=0.8)
  par(bty = "l",cex.lab = 2)
  plot(x = clc_d_mg$lag, clc_d_mg$fac, xlab = "", ylab = "",
       type = "h", axes = FALSE, ylim = c(-2/sqrt(T) - 0.1, 1), lwd = 2, main = "FAC")
  lines(x = c(-3,lags), y = c(0,0),lwd = 2)
  lines(x = c(-3,lags), y = c(-1/T-2/sqrt(T), -1/T-2/sqrt(T)), col = "red", lty = 2, lwd = 2)
  lines(x = c(-3,lags), y = c(-1/T+2/sqrt(T), -1/T+2/sqrt(T)), col = "red", lty = 2, lwd = 2)
  ...
  par(bty = "l",cex.lab = 2)
  plot(x = clc_d_mg$lag[2:25], clc_d_mg$facp[2:25], xlab = "", ylab = "",
       type = "h", axes = FALSE, ylim = c(-2/sqrt(T) - 0.1, 1), lwd = 2, main = "FACP")
  lines(x = c(-3,lags), y = c(0,0),lwd = 2)
  lines(x = c(-3,lags), y = c(-2/sqrt(T),-2/sqrt(T)), col = "red", lty = 2, lwd = 2)
  lines(x = c(-3,lags), y = c(2/sqrt(T), 2/sqrt(T)), col = "red", lty = 2, lwd = 2)
...
rho1_d_mg <- clc_d_mg$fac[2]
rho2_d_mg <- clc_d_mg$fac[3]

rho1_o_mg <- clc_o_mg$fac[2]
rho2_o_mg <- clc_o_mg$fac[3]

#AR(1)
phi1_ar1_dmg = clc_d_mg$fac[lag==1]
phi1_ar1_omg = clc_o_mg$fac[lag==1]

par_ar1_mg <- data.frame(phi1_ar1_dmg, phi1_ar1_omg)

#MA(1):
theta1_ma1_dmg <- (1 - sqrt(1 - 4 * rho1_d_mg^2)) / (2 * rho1_d_mg)

theta1_ma1_omg <- (1 - sqrt(1 - 4 * rho1_o_mg^2)) / (2 * rho1_o_mg)

par_ma1_mg <-data.frame(theta1_ma1_dmg,theta1_ma1_omg)

#AR(2)

phi1_ar2_dmg <- (rho1_d_mg - rho1_d_mg * rho2_d_mg) / (1 - rho1_d_mg^2)
phi2_ar2_dmg <- (rho2_d_mg - rho1_d_mg^2) / (1 - rho1_d_mg^2)

phi1_ar2_omg <- (rho1_o_mg - rho1_o_mg * rho2_o_mg) / (1 - rho1_o_mg^2)
phi2_ar2_omg <- (rho2_o_mg - rho1_o_mg^2) / (1 - rho1_o_mg^2)

par_ar2_mg <-data.frame(phi1_ar2_dmg,phi2_ar2_dmg,phi1_ar2_omg,phi2_ar2_omg)
...
par_ma2_mg<-data.frame("theta1_ma2_dmg"=solucao$x[1],"theta2_ma2_dmg"=solucao$x[2])

#ARMA (1,1) - apenas desocupados

phi1_arma11_dmg <- rho2_d_mg/rho1_d_mg
...
rho1 <- rho1_d_mg
phi1 <- phi1_arma11_dmg

theta1_arma11_dmg <- arma11_theta1(rho1, phi1)

par_arma11_mg <-data.frame(phi1_arma11_dmg,theta1_arma11_dmg)

### AR(5)
#Desocupados
rho3_d_mg <- clc_d_mg$fac[4]
rho4_d_mg <- clc_d_mg$fac[5]
rho5_d_mg <- clc_d_mg$fac[6]

rho_d <- c(rho1_d_mg, rho2_d_mg, rho3_d_mg, rho4_d_mg, rho5_d_mg)
R_d <- matrix(1, nrow = 5, ncol = 5)
for (i in 1:5) {
  for (j in 1:5) {
    if (i != j) {
...
rho_d_vec <- rho_d # Vetor
phi_d <- solve(R_d, rho_d_vec)
ar5_d_mg<-as.data.frame(phi_d)

#Ocupados
rho3_o_mg <- clc_o_mg$fac[4]
rho4_o_mg <- clc_o_mg$fac[5]
rho5_o_mg <- clc_o_mg$fac[6]

rho_o <- c(rho1_o_mg, rho2_o_mg, rho3_o_mg, rho4_o_mg, rho5_o_mg)
R_o <- matrix(1, nrow = 5, ncol = 5)
for (i in 1:5) {
  for (j in 1:5) {
    if (i != j) {
...
rho_o_vec <- rho_o # Vetor
phi_o <- solve(R_o, rho_o_vec)
ar5_o_mg<-as.data.frame(phi_o)

par_ar5_mg<-cbind(ar5_d_mg,ar5_o_mg)

params_mg <- list("dbmg"=dbmg,"calculos_desocupada_mg"=clc_d_mg,
                  "calculos_ocupada_mg"=  clc_o_mg, 
                  "mod_ar1" = par_ar1_mg, "mod_ar2"=par_ar2_mg,
                  "mod_ma1"=par_ma1_mg,"mod_ma2"=par_ma2_mg, "mod_arma11"=par_arma11_mg, "mod_ar5"=par_ar5_mg)

saveRDS(params_mg,file = "D:/FJP2425/Programacao/data/pseudoerros_8reg/09_params_mg.rds")
