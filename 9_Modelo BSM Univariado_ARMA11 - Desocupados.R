...
# Sobre modelos ARMA:
  # Problemas em conseguir rodar a função no código atual
    # NAs
  # Reescrita da função: ajustes nas matrizes G e W
    # Lembrando, no pacote dlm temos que:
      # G -> matriz de transição
      # W -> matriz de variâncias
  # Fiz os ajustes conforme as matriizes da tese e o livro do harvey 1989 (p. 103 e 104)
...
source("data/funcoes/08_teste_bsm_error_ARMA11.R")
...
modelos_bsm_error_ini <- parLapply(cl, 1:nrow(grid_error), function(i) {
  tryCatch(f.teste_bsm_error(y, grid_error[i, ]), error = function(e) {
    rep(NA, 7)
  })
})
stopCluster(cl)
...
b <- cbind(
  ...
  do.call(rbind, lapply(1:nrow(grid_error), function(i) {
    tryCatch({
      params <- round(exp(modelos_bsm_error_ini[[i]][["fit"]][["par"]]), 4)
