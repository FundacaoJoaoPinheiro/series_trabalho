...
calcula_ocup_desocup_k <- function(mesano) {
  ...
  # Calcula estimativas por região e rotação
  estimativas <- svyby(
    ~ocupada + desocupada,
    by = ~regioes + V1016,
    subset(pnadc, UF == "31"),
  ...
  # Calcula o total de MG considerando os grupos de rotação
  total_mg <- svyby(
    ~ocupada + desocupada,
    by = ~V1016,
    subset(pnadc, UF == "31"),
  ...
  total_mg$periodo <- paste0(substr(mesano, 2, 5), "_0", substr(mesano, 1, 1))
  
  # Combina os resultados
  estimativas$periodo <- paste0(substr(mesano, 2, 5), "_0", substr(mesano, 1, 1))
  estimativas <- rbind(estimativas, total_mg) %>%
    arrange(regioes, V1016)
  
  # Salva os resultados
  saveRDS(estimativas, paste0("D:/FJP2425/Programacao/data/rotacao8reg/resultados_0", mesano, ".RDS"))
...
dados<-list.files("data/rotacao8reg", pattern = "\\.RDS$", full.names = TRUE)

lista_dados<-lapply(dados, readRDS)

baserot8reg<-funorg(lista_dados)

#saveRDS(baserot8reg, file = "D:/FJP2425/Programacao/data/baserot8reg.rds")

datarot<-readRDS("D:/FJP2425/Programacao/data/baserot8reg.RDS")
...
saveRDS(basetotal, file = "D:/FJP2425/Programacao/data/basealinhada_8reg.rds")

dados<-readRDS("D:/FJP2425/Programacao/data/basealinhada_8reg.rds")
