...
f.smooth_ar1<-function(y,i0){
  ...
  m$GG[6,6] <- phi1_ar1
  W = matrix(0,6,6)
  W[1, 1] <- 10^(-6) # Modelo de tendência suavizada
  W[2, 2] <- exp(params[1])
  W[3, 3] <- exp(params[2])
  W[6, 6] <- exp(params[4])
  m$W <- W
...
c_sinal_bsm_error <- matrix(c(1,0,1,0, # somando tendência e os seis componentes sazonais
                     1,0),1,6) # indica com 1 qual a coluna(estado) quer considerar*caso trigonométrico
c_seasonal_bsm_error <- matrix(c(0,0,1,0, # somando os seis componentes sazonais
                         1,0),1,6) # indica com 1 qual a coluna(estado) quer considerar*caso trigonométrico
# c_sinal_bsm_error <- matrix(c(1,0,1,1, # somando tendência e os seis componentes sazonais
#                       1,0),1,6) # indica com 1 qual a coluna(estado) quer considerar*caso normal
# c_seasonal_bsm_error <- matrix(c(0,0,1,1, # somando os seis componentes sazonais
#                          1,0),1,6) # indica com 1 qual a coluna(estado) quer considerar*caso normal
...
modelo$se.sampling_error<- se.mat_bsm_error[,6]
...
# cria vertores indicadores para soma de estados
 c_sinal_bsm_error.sm <- matrix(c(1,0,1,0, # somando tendência e os seis componentes sazonais
                       1,0),1,6) # indica com 1 qual a coluna(estado) quer considerar*caso trigonométrico
 c_seasonal_bsm_error.sm <- matrix(c(0,0,1,0, # somando os seis componentes sazonais
                          1,0),1,6) # indica com 1 qual a coluna(estado) quer considerar*caso trigonométrico
#c_sinal_bsm_error.sm <- matrix(c(1,0,1,1, # somando tendência e os seis componentes sazonais
#                      1,0),1,6) # indica com 1 qual a coluna(estado) quer considerar*caso normal
#c_seasonal_bsm_error.sm <- matrix(c(0,0,1,1, # somando os seis componentes sazonais
#                         1,0),1,6) # indica com 1 qual a coluna(estado) quer considerar*caso normal
...
modelo$se.sm.sampling_error<- se.mat_bsm_error.sm[,6]
