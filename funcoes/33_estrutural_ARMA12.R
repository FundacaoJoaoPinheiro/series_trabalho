...
f.estrutural_arma12<-function(y,i0){
  modelo<- list("fn"=function(params){
    m <- dlmModPoly(2) + dlmModTrig(4) + dlmModReg(se_db, addInt = FALSE)
    m$FF <- cbind(m$FF, matrix(0, nrow = 1, ncol = 2))
    m$GG <- rbind(m$GG, matrix(0, nrow = 2, ncol = ncol(m$GG)))
    m$GG <- cbind(m$GG, matrix(0, nrow = nrow(m$GG), ncol = 2)) 
    m$GG[6,6] <- phi1_arma12
    m$GG[6,7] <- theta1_arma12         
    m$GG[7,6] <- 0
    m$GG[7,7] <- 0
    m$GG[6,8] <- 0
    m$GG[7,8] <- theta2_arma12
    m$GG[8,7] <- 0
    m$GG[8,8] <- 0
  ...
  c_sinal_bsm_error <- matrix(c(1,0,1,0,1,0,0,0),1,8)
  c_seasonal_bsm_error <- matrix(c(0,0,1,0,1,0,0,0),1,8)
  ...
  # cria vetores indicadores para soma de estados
  c_sinal_bsm_error.sm <- matrix(c(1,0,1,0,1,0,0,0),1,8)
  c_seasonal_bsm_error.sm <- matrix(c(0,0,1,0,1,0,0,0),1,8)
