...
f.estruturaldummy_arma11<-function(y,i0){
  modelo<- list("fn"=function(params){
    ...
    m$GG[6,6] <- phi1_arma11
    m$GG[6,7] <- theta1_arma11        
    m$GG[7,6] <- 0
    m$GG[7,7] <- 0 
    ...
    W[2,2] <- exp(params[2])
    W[3,3] <- exp(params[3])
    W[6,6] <- exp(params[5]) # Variância do e_t
    W[7,7] <- 0
    m$W <- W
  ...
  c_sinal_bsm_error <- matrix(c(1,0,1,1,1,0,0),1,7)
  c_seasonal_bsm_error <- matrix(c(0,0,1,1,1,0,0),1,7)
  ...
  # cria vetores indicadores para soma de estados 
  #c_sinal_bsm_error.sm <- matrix(c(1,0,1,0,1,0,0),1,7)  # trigonométrico
  #c_seasonal_bsm_error.sm <- matrix(c(0,0,1,0,1,0,0),1,7) # trigonométrico
  
  c_sinal_bsm_error.sm <- matrix(c(1,0,1,1,1,0,0),1,7)
  c_seasonal_bsm_error.sm <- matrix(c(0,0,1,1,1,0,0),1,7)
